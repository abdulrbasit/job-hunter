...
- Use `job-hunter agent-context lifecycle --job <slug>` after import. If it reports a failed fetch, resolve that before scoring.
...
6. On confirmation:
   ...
   - Ask before final commit; if confirmed, run `job-hunter finalize-run --mode interactive --message "chore(jobs): tailor <slug>"` and add `--push` only when the user explicitly asks to push.
