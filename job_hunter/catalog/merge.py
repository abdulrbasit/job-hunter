...
def effective_companies(job_hunter_config: dict[str, Any], career_pages_data: dict[str, Any]) -> list[dict[str, Any]]:
    """Eligible bundled catalog companies + enabled custom companies.

    A custom entry whose career_url matches a bundled company wins — the bundled
    entry is skipped rather than duplicated.
    """
    catalog_settings = career_pages_data.get("catalog") or {}
    catalog_enabled = catalog_settings.get("enabled", True)
    disabled_ids = {str(company_id) for company_id in catalog_settings.get("disabled_company_ids", []) or []}
    ...
    excluded_industry_ids = _excluded_industry_ids(
        (job_hunter_config.get("exclusions", {}) or {}).get("industries", []) or []
    )
    ...
    if catalog_enabled:
        for company in load_companies():
            if not _is_eligible(company, enabled_countries, excluded_industry_ids, disabled_ids):
                continue
