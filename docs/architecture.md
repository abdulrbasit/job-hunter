...
| `core/` | Cross-package utilities and package-owned built-in quality filters, including offline language detection (`core/language.py`) and offline experience-range detection (`core/experience.py`), both consumed by `sources/policy.py`'s screening gate |
