...
(public entry point, never raises — wraps `_fetch` in try/except, returns `[]` on failure). Every adapter
accepts the shared `SearchParams` contract — enforced today by
`tests/test_source_contracts.py::test_all_adapters_accept_shared_search_contract`.

**`sources/ats/` removed (Phase 12).** A parallel per-company ATS dispatcher (`dispatch.py` + 11 platform
adapters: greenhouse, lever, bamboohr, smartrecruiters, workable, ashby, hibob, personio, recruitee,
teamtailor, workday — ~1,400 lines + `test_ats.py`) existed since the initial commit but was never wired
into `orchestrator.py`, `job_boards.py`, or `career_pages/` — confirmed zero production callers via
exhaustive grep. Superseded by `sources/search/ats_discovery.py`'s search-based ATS discovery, which is
the active path. `sources/ats_urls.py` and `sources/ats_slugs.py` (used by `career_pages/`, `jd_fetcher.py`,
`orchestrator.py`, `search/ats_discovery.py`) are unrelated top-level files and were not touched.
...
| `sources/ats/` (whole package: `dispatch.py`, `_base.py`, 11 platform adapters) | deleted | dead code — zero production callers, confirmed via exhaustive grep; superseded by `sources/search/ats_discovery.py` | n/a | `test_ats.py` deleted; 2 tests in `test_sources.py` that exercised it via the package facade removed too | none | done (Phase 12) — see §5 |
...
|---|---|---|
| `JobSourceAdapter.name` (alias for `source_name`) | removed | **stale row correction (Phase 12)** — this alias was already removed in an earlier phase (not documented at the time); confirmed zero references to `.name` on an adapter anywhere in code or tests as of Phase 12's audit | n/a | none needed | none | done (already correct, no action needed) |
...
| `pipeline/orchestrator.py::_build_parser` + `argparse` `__main__` entry point | n/a | **stale row correction (Phase 12)** — `pipeline/orchestrator.py` no longer exists; its content was redistributed into `runner.py`/`modes/*.py`/`stages/*.py` in Phase 5, and the argparse entry point went with it (confirmed: zero `import argparse` anywhere under `pipeline/`) | n/a | none needed | none | done (already correct, no action needed) |
| `linkedin/{ideas,drafts,engagement}.py::main()` + `argparse` + `if __name__ == "__main__"` | removed | **found during Phase 12's argparse sweep, not in this table originally** — the Typer CLI (`cli/commands/linkedin.py`) calls `generate`/`draft`/`discover` directly, never each module's own `main()`; confirmed zero references to `python -m job_hunter.linkedin.*` anywhere (scripts, docs, workflows) | n/a | none tested `main()` directly | none | done (Phase 12) |
...
| Old config keys (`about_me`, `sources`, `secrets`, top-level `tailoring`/`cover_letter`, `exclusions.{senior_flags,stale_indicators,url_patterns,language_indicators}`, `scoring.prompt_context`, rich `linkedin.*` keys) | rejected by `config/removed_keys.py::reject_removed_user_config` | **keep the rejection guard** — the keys themselves are already gone from the schema; the guard is the intentional break-loudly mechanism for anyone upgrading from a pre-cutoff config, not itself removable. Phase 12 removed `config/loader.py::_reject_removed_user_config`, a dead underscore-alias re-export of this function with zero remaining callers — the real function (public name, `removed_keys.py`) is untouched |
...
| `.gemini/` obsolete agent-CLI mirror dir | `workspace/assets.py::_OBSOLETE_CLI_DIRS` (path renamed, Phase 11) | **keep temporarily** — cleanup-on-update code for any workspace still carrying the old dir; no telemetry to know when it's safe to drop, keep indefinitely (near-zero cost); reconfirmed still present and correct in Phase 12's audit |
...
| `pipeline/enrichment.py::JD_STATUS_SPARSE` (unused alias for `JD_STATUS_THIN`) | removed | **found during Phase 12's broad search** — confirmed zero references to the constant or the string `"sparse"` anywhere outside its own definition | n/a | none needed | none | done (Phase 12) |
| `pipeline/cover_writer.py::_config_section`'s "legacy top-level" branch (`if name in config: return config.get(name)`) | simplified to the nested-only lookup | **found during Phase 12's broad search** — `header`/`closing` can never appear as top-level `job_hunter.yml` keys: the schema's top-level `additionalProperties: false` only allows `mode, profile, job_titles, regions, exclusions, scoring, llm`, and `cover_letter` itself is in `_TOP_LEVEL_REMOVED` (code-owned only, merged in via `COVER_LETTER_DEFAULTS`) — the legacy branch was provably unreachable | n/a | `test_cover_writer.py` (unaffected — exercises the reachable branch only) | none | done (Phase 12) |
