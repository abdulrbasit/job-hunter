...
- Autonomous user uses the standard install and runs the same workflow in `llm-api` mode.
...
| Bundled LLM SDK guidance | Commit `ecb2a11`; 3 provider cases failed on old install messages | `tests/test_llm_client.py` passes |
...
| Standard installation supports both agent and LLM API modes | `tests/test_packaging.py` |
