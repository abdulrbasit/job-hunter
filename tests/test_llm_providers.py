...
def test_resolve_provider_uses_role_override() -> None:
    llm_config = {"default_provider": "anthropic", "providers": {"tailoring": "openai"}}

    assert resolve_provider("tailoring", llm_config) == "openai"


def test_resolve_provider_falls_back_to_default() -> None:
    llm_config = {"default_provider": "anthropic", "providers": {}}

    assert resolve_provider("scoring", llm_config) == "anthropic"
...
def test_resolve_model_config_requires_explicit_role_keys() -> None:
    with pytest.raises(KeyError, match="llm.models.jd_extraction"):
        resolve_model_config(
            "jd_extraction",
            api_config={"llm": {"models": {}, "max_tokens": {}}},
        )
