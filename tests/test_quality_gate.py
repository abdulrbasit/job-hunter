...
def test_negative_title_term_reduces_score_with_title_weight():
    ...
    assert score_quality_signals(job_title, config)["_pre_llm_score"] == -5.0
    assert score_quality_signals(job_snippet, config)["_pre_llm_score"] == -2.0
...
def test_max_before_llm_scoring_caps_kept_jobs():
    jobs = [_job(title=f"Job {i}") for i in range(100)]
    kept, rejected = apply_pre_llm_quality_gate(jobs, _config(max_before_llm_scoring=20))
    assert len(kept) == 20
...
def test_min_pre_score_rejects_jobs_below_threshold():
    ...
    kept, rejected = apply_pre_llm_quality_gate([job_bad, job_ok], config)
...
def test_min_pre_score_keeps_jobs_at_exact_threshold():
    ...
    kept, rejected = apply_pre_llm_quality_gate([job], config)
    assert len(kept) == 1
    assert len(rejected) == 0


def test_apply_pre_llm_quality_gate_respects_both_min_score_and_cap():
    ...
    kept, rejected = apply_pre_llm_quality_gate(jobs + bad_jobs, config)
    assert len(kept) == 2
...
def test_disabled_gate_returns_all_jobs_unchanged_pre_llm():
    jobs = [_job(title=f"Job {i}") for i in range(50)]
    kept, rejected = apply_pre_llm_quality_gate(jobs, _config(enabled=False))
    assert kept is jobs
    assert rejected == []
