"""Tests for orchestrator._write_company_research()."""
...
def test_write_company_research_creates_file(tmp_path: Path) -> None:
    with (
        patch("job_hunter.pipeline.orchestrator.get_config", return_value={"job_titles": ["Product Manager"]}),
        patch(_LLM_STAGE) as mock_cls,
    ):
        mock_cls.return_value.complete.return_value = "## Product & Business\nAcme builds widgets."
        orchestrator._write_company_research(_job(), tmp_path)
...
def test_write_company_research_file_starts_with_company_header(tmp_path: Path) -> None:
    with (
        patch("job_hunter.pipeline.orchestrator.get_config", return_value={}),
        patch(_LLM_STAGE) as mock_cls,
    ):
        mock_cls.return_value.complete.return_value = "Some content."
        orchestrator._write_company_research(_job(company="MegaCorp"), tmp_path)
...
def test_write_company_research_does_not_raise_on_llm_failure(tmp_path: Path) -> None:
    with (
        ...
        orchestrator._write_company_research(_job(), tmp_path)  # must not raise
