...
def test_positive_title_term_increases_score():
    ...
    result = pre_score_job(job, _config(positive_terms=["python"]))
...
def test_negative_title_term_reduces_score_with_title_weight():
    ...
    assert pre_score_job(job_title, config)["_pre_llm_score"] == -5.0
    assert pre_score_job(job_snippet, config)["_pre_llm_score"] == -2.0
...
def test_snippet_negative_term_uses_snippet_weight():
    ...
    result = pre_score_job(job, _config(negative_terms=["german"]))
...
def test_pre_llm_reasons_records_negative_match():
    ...
    result = pre_score_job(job, _config(negative_terms=["manager"]))
...
def test_pre_llm_score_and_reasons_fields_always_present():
    job = _job()
    result = pre_score_job(job, _config())
...
def test_empty_terms_lists_score_all_jobs_zero():
    ...
    result = pre_score_job(job, _config(positive_terms=[], negative_terms=[]))
...
def test_max_before_enrichment_caps_kept_jobs():
    ...
    kept, rejected = apply_pre_enrichment_gate(jobs, _config(max_before_enrichment=10))
    assert len(kept) == 10
...
def test_max_before_enrichment_does_not_apply_min_pre_score():
    ...
    kept, rejected = apply_pre_enrichment_gate([job], config)
    assert len(kept) == 1
...
def test_disabled_gate_returns_all_jobs_unchanged_pre_enrichment():
    ...
    kept, rejected = apply_pre_enrichment_gate(jobs, _config(enabled=False))
    assert kept is jobs
    assert rejected == []
...
def test_max_before_llm_scoring_caps_kept_jobs():
    jobs = [_job(title=f"Job {i}") for i in range(100)]
    kept, rejected = apply_pre_llm_gate(jobs, _config(max_before_llm_scoring=20))
    assert len(kept) == 20
...
def test_min_pre_score_rejects_jobs_below_threshold():
    ...
    kept, rejected = apply_pre_llm_gate([job_bad, job_ok], config)
...
def test_min_pre_score_keeps_jobs_at_exact_threshold():
    ...
    kept, rejected = apply_pre_llm_gate([job], config)
    assert len(kept) == 1
    assert len(rejected) == 0
...
def test_apply_pre_llm_gate_respects_both_min_score_and_cap():
    ...
    kept, rejected = apply_pre_llm_gate(jobs + bad_jobs, config)
    assert len(kept) == 2
...
def test_disabled_gate_returns_all_jobs_unchanged_pre_llm():
    jobs = [_job(title=f"Job {i}") for i in range(50)]
    kept, rejected = apply_pre_llm_gate(jobs, _config(enabled=False))
    assert kept is jobs
    assert rejected == []
